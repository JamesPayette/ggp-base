...
public class CompulsiveDeliberatorWayne extends GGPlayer {
	...
	public static void main(String[] args) {
		Player.initialize(new CompulsiveDeliberatorWayne().getName());
	}
	...
	public Move play(long timeout)
		...
		for (Move move : legalMoves) {
			int score = deliberate(move, role, state, machine);
			if (score > bestScore) {
				bestScore = score;
				chosenMove = move;
			}
	...
	private int deliberate(Move move, Role role, MachineState state, StateMachine machine)
			throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException{
		// Execute the move. If in a goal state, return the reward.
		MachineState nextState = findNext(Arrays.asList(move), state, machine);
		if (findTerminalp(nextState, machine)) {
			return findReward(role, nextState, machine);
		}

		// If not in a goal state, recurse on the list of legal moves in the new state.
		int bestScore = Integer.MIN_VALUE;
		List<Move> legalMoves = findLegals(role, nextState, machine);
		for (Move nextMove : legalMoves) {
			int score = deliberate(nextMove, role, nextState, machine);
			if (score > bestScore) {
				bestScore = score;
			}
		}
