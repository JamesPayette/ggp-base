...
public class MobilityHeuristicMinimaxPlayerWayne extends GGPlayer {
	...
	public static void main(String[] args) {
		Player.initialize(new MobilityHeuristicMinimaxPlayerWayne().getName());
	}
	...
	private int maxNode(Role role, MachineState state, StateMachine machine,
		...
		if (timeout - System.currentTimeMillis() < timeBuffer || depth >= depthLimit) {
			return 100 * findLegals(role, state, machine).size() / findActions(role, machine).size();
		}
