...
public class NodeMinimaxPlayerWayne extends GGPlayer {

	// Hunter 250000 nodes
	//
	final static int nodeLimit = 250000;
	...
	public static void main(String[] args) {
		Player.initialize(new NodeMinimaxPlayerWayne().getName());
	}
	...
	public Move play(long timeout)
		...
		for (Move move : legalMoves) {
			int score = minNode(move, role, state, machine, nodesExplored);
			if (score > bestScore) {
				bestScore = score;
				chosenMove = move;
			}
	...
	private int minNode(Move move, Role role, MachineState state, StateMachine machine, int[] nodesExplored)
			throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException {
		...
		for (List<Move> jointMove : jointMoves) {
			...
			int result = maxNode(role, newState, machine, nodesExplored);
			if (result < worstScore) {
				worstScore = result;
			}

			if (nodesExplored[0] >= nodeLimit) {
				return worstScore;
			}
		}
	...
	private int maxNode(Role role, MachineState state, StateMachine machine, int[] nodesExplored)
			throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException{
		nodesExplored[0] += 1;
