...
public class MinimaxPlayerWayne extends GGPlayer {
	...
	public static void main(String[] args) {
		Player.initialize(new MinimaxPlayerWayne().getName());
	}
	...
	public Move play(long timeout)
		...
		for (Move move : legalMoves) {
			int score = minNode(move, role, state, machine);
			if (score > bestScore) {
				bestScore = score;
				chosenMove = move;
			}
	...
	private int minNode(Move move, Role role, MachineState state, StateMachine machine)
		...
		for (List<Move> jointMove : jointMoves) {
			...
			int result = maxNode(role, newState, machine);
