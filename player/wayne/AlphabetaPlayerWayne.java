...
public class AlphabetaPlayerWayne extends GGPlayer {
	...
	public static void main(String[] args) {
		Player.initialize(new AlphabetaPlayerWayne().getName());
	}
	...
	public Move play(long timeout)
		...
		for (Move move : legalMoves) {
			int score = minNode(move, role, state, machine, alpha, beta);
			if (score > bestScore) {
				bestScore = score;
				chosenMove = move;
			}
	...
	private int minNode(Move move, Role role, MachineState state, StateMachine machine, int alpha, int beta)
			throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException {
		...
		for (List<Move> jointMove : jointMoves) {
			...
			int result = maxNode(role, newState, machine, alpha, beta);
	...
	private int maxNode(Role role, MachineState state, StateMachine machine, int alpha, int beta)
			throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException{
		...
		for (Move move : legalMoves) {
			int result = minNode(move, role, state, machine, alpha, beta);
