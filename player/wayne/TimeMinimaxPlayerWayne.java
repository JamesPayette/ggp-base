...
public class TimeMinimaxPlayerWayne extends GGPlayer {
	...
	public static void main(String[] args) {
		Player.initialize(new TimeMinimaxPlayerWayne().getName());
	}
	...
	public Move play(long timeout)
		...
		for (Move move : legalMoves) {
			int score = minNode(move, role, state, machine, timeout, nodesExplored);
			if (score > bestScore) {
				bestScore = score;
				chosenMove = move;
			}
	...
	private int minNode(Move move, Role role, MachineState state, StateMachine machine, long timeout, int[] nodesExplored)
			throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException {
		...
		for (List<Move> jointMove : jointMoves) {
			...
			int result = maxNode(role, newState, machine, timeout, nodesExplored);
	...
	private int maxNode(Role role, MachineState state, StateMachine machine, long timeout, int[] nodesExplored)
			throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException{
		nodesExplored[0] += 1;
