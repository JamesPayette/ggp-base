...
public class DepthMinimaxPlayerWayne extends GGPlayer {

	// Depth 11 on Hunter
	// Depth 6 on Kono
	final static int depthLimit = 6;
	...
	public static void main(String[] args) {
		Player.initialize(new DepthMinimaxPlayerWayne().getName());
	}
	...
	public Move play(long timeout)
		...
		for (Move move : legalMoves) {
			int score = minNode(move, role, state, machine, 0);
			if (score > bestScore) {
				bestScore = score;
				chosenMove = move;
			}
	...
	private int minNode(Move move, Role role, MachineState state, StateMachine machine, int level)
			throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException {
		...
		for (List<Move> jointMove : jointMoves) {
			...
			int result = maxNode(role, newState, machine, level + 1);
	...
	private int maxNode(Role role, MachineState state, StateMachine machine, int level)
			throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException{
		...
		for (Move move : legalMoves) {
			int score = minNode(move, role, state, machine, level);
			if (score > bestScore) {
				bestScore = score;
			}
		}
